...
This is the building block for all other abstractions (remote file objects, callbacks, etc.).
```rs
#[remote_interface]
pub trait SimpleOps {
...
- Modify the original interface definition to include a mutable receiver (`&mut self`), for optional persistence. The implementation does not need to take into account any network-related logic.
    ```rs
...
with the ability to marshall and unmarshall any arbitrary data type.
This brings a lot of flexibility to defining custom payloads.
```rs
...
This circumvents the need to compress the data, as the serialized data is already compressed.
